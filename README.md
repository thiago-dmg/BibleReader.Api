...
Igual ao **Minha Rotina**: só **`deploy.yml`** — em **push** para `main`/`master` (ou manual) faz restore, publish, envia para a VPS e reinicia o serviço systemd.
...
O **Actions só publica a API** e reinicia o serviço; o **SQL Server** tem de existir e a connection string tem de estar correta. Na primeira subida com sucesso, o `Program.cs` aplica **migrations** e o **seed** da Bíblia (se a app arrancar sem erro).
